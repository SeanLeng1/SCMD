...
def _hparams(algorithm, dataset, random_seed):
    ...
    _hparam('SMA', True, lambda r: bool(r.choice([True, True])))
    ...
    # ---------------------------------------------
    # add hyperparameter for CLIP
    elif algorithm == "CLIP":
        _hparam('clip_backbone', 'ViT-B/32', lambda r: r.choice(['ViT-B/16']))
        _hparam('prompt', 'class_name', lambda r: r.choice(['class_name', 'domain_name']))
        _hparam('clip_transform', True, lambda r: bool(r.choice([True, True])))

    elif algorithm == "linear_probe_CLIP" or algorithm == "fine_tune_CLIP":
        _hparam('clip_backbone', 'RN50', lambda r: r.choice(['RN50']))
        _hparam('lp_lr', 5e-3, lambda r: 2 ** r.uniform(-2, 0))

    # feature_based_KD
    elif algorithm == "feature_based_KD":
        _hparam('clip_backbone', 'ViT-B/32', lambda r: r.choice(['ViT-B/32']))
        _hparam('student_model', 'resnet', lambda r: r.choice(['resnet']))
        _hparam('lambda', 1, lambda r: r.uniform(0.5, 1))
        _hparam('linear_projector', False, lambda r: bool(r.choice([False, False])))
        _hparam('method', 'linear', lambda r: r.choice(['linear', 'fitnet']))

    elif algorithm == "SCMD" or algorithm == 'SCMD_no_CM':
        _hparam('clip_backbone', 'ViT-B/32', lambda r: r.choice(['ViT-B/32']))
        _hparam('student_model', 'resnet', lambda r: r.choice(['resnet']))
        _hparam('lambda1', 0.5, lambda r: r.uniform(0.5, 1.0))
        _hparam('lambda2', 0.5, lambda r: r.uniform(0.5, 1.0))
        _hparam('last_k_epoch', 0.25, lambda r: r.uniform(0.2, 0.4))
        _hparam('worst_case_p', 0.2, lambda r: r.choice([0.2, 0.25, 0.3]))
        _hparam('temperature', 3, lambda r: r.uniform(2., 5.))

    elif algorithm == "SCMD_focal":
        _hparam('clip_backbone', 'ViT-B/32', lambda r: r.choice(['ViT-B/32']))
        _hparam('student_model', 'resnet', lambda r: r.choice(['resnet']))
        _hparam('lambda1', 0.5, lambda r: r.uniform(0.5, 1.0))
        _hparam('lambda2', 0.5, lambda r: r.uniform(0.5, 1.0))
        ...
        _hparam('worst_case_p', 0.2, lambda r: r.choice([0.2, 0.25, 0.3]))
        _hparam('temperature', 3, lambda r: r.uniform(2., 5.))
        _hparam('gamma', 2.0, lambda r: r.uniform(1, 5))
        _hparam('alpha', 0.25, lambda r: r.uniform(0.1, 0.3))

    elif algorithm == "Baseline":
        _hparam('clip_backbone', 'ViT-B/32', lambda r: r.choice(['ViT-B/32']))
        _hparam('rsc_f_drop_factor', 1 / 4, lambda r: r.choice([0.2, 0.25, 0.3]))
        _hparam('rsc_b_drop_factor', 1 / 4, lambda r: r.choice([0.2, 0.25, 0.3]))
        _hparam('student_model', 'resnet', lambda r: r.choice(['resnet']))
        _hparam('lambda1', 0.5, lambda r: r.uniform(0.5, 1.0))
        _hparam('temperature', 3, lambda r: r.uniform(2., 5.))
        ...
        _hparam('lambda1', 0.5, lambda r: r.uniform(0.5, 1.0))
        _hparam('temperature', 3, lambda r: r.uniform(2., 5.))
        _hparam('attack_size', 32, lambda r: int(2**r.uniform(3, 5)))

    elif algorithm == "RKD":
        _hparam('clip_backbone', 'ViT-B/32', lambda r: r.choice(['ViT-B/32']))
        _hparam('student_model', 'resnet', lambda r: r.choice(['resnet']))
        _hparam('w_dist', 25.0, lambda r: r.uniform(20, 25.0))
        _hparam('w_angle', 50.0, lambda r: r.uniform(30, 50.0))
        _hparam('lambda1', 1.0, lambda r: r.uniform(0.5, 1.0))

    elif algorithm == "FitNet":
        _hparam('clip_backbone', 'ViT-B/32', lambda r: r.choice(['ViT-B/32']))
        _hparam('student_model', 'resnet', lambda r: r.choice(['resnet']))
        _hparam('lambda1', 1.0, lambda r: r.uniform(0.5, 1.0))
    ...
    if dataset in SMALL_IMAGES:
        ...
        _hparam('lr', 5e-5, lambda r: r.choice([3e-5, 5e-5]))
        

    if dataset in SMALL_IMAGES:
        _hparam('weight_decay', 0., lambda r: 0.)
    else:
        #_hparam('weight_decay', 0., lambda r: 10**r.uniform(-6, -4)) # (-6, -2) -> (-6, -4)
    ...
    if dataset in SMALL_IMAGES:
        ...
        # modified to avoid OOM
        _hparam('batch_size', 32, lambda r: int(2**r.uniform(4.9, 5.1)))
